...
def update():
    ...
    for ball in balls:  #balls는 단순히 ball을 관리하는 리스트임
        if collide(boy, ball):  # 컬리젼 함수의 리턴 값이 트루이면
            ...
            game_world.remove_object(ball)  # 더이상 충돌을 처리할 필요는 없으므로 게임월드에서도 제거함.
            #  충돌한 것 같은데 충돌이 안된다? 충돌을 검사하는지? , 그에대한 응답을 설정했는지.
    for ball in balls:
        if collide(grass, ball):  # 땅과 공도 충돌처리
            ball.stop()

    # delay(0.99) 딜레이가 길어질 수록 터널릴 효과
    # 따로 분리하는게(소년, 공), (땅, 공) 가독성 차원에서 더 좋음.
